...
<form method="GET" action="{{ route('catalog.index') }}" class="flex gap-4 mb-6">
    ...
    <select name="kategori" class="border p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7A2E2E]">
...
<a href="{{ route('catalog.show', $book->id) }}"
   class="bg-white p-4 rounded-2xl shadow-md border border-gray-100
          hover:shadow-xl hover:-translate-y-1 hover:border-[#7A2E2E]
          active:scale-95 transition duration-300 block">
    ...
    <!-- CATEGORY -->
    <div class="text-xs bg-gray-100 px-3 py-1 rounded-full mb-2 inline-block">
        {{ $book->kategori->nama_kategori ?? '-' }}
    </div>

    <!-- TITLE -->
    <div class="font-semibold text-[#7A2E2E] line-clamp-2">
        {{ $book->judul }}
    </div>
